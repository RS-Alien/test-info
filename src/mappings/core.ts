...
export function handleSync(event: Sync): void {
  ...
  if (bundle.ethPrice.notEqual(ZERO_BD)) {
    trackedLiquidityETH = getTrackedLiquidityUSD(pair.reserve1, token1 as Token, pair.reserve0, token0 as Token).div(
      bundle.ethPrice
    )
  } else {
  ...
  pair.reserveETH = pair.reserve1
    .times(token1.derivedETH as BigDecimal)
    .plus(pair.reserve0.times(token0.derivedETH as BigDecimal))
...
export function handleMint(event: Mint): void {
  ...
  let bundle = Bundle.load('1')
  let amountTotalUSD = token0.derivedETH
    .times(token0Amount)
    .plus(token1.derivedETH.times(token1Amount))
...
export function handleSwap(event: Swap): void {
  ...
  let derivedAmountETH = token0.derivedETH
    .times(amount0Total)
    .plus(token1.derivedETH.times(amount1Total))
    .div(BigDecimal.fromString('2'))
  let derivedAmountUSD = derivedAmountETH.times(bundle.ethPrice)

  // only accounts for volume through white listed tokens
  let trackedAmountUSD = getTrackedVolumeUSD(amount1Total, token1 as Token, amount0Total, token0 as Token, pair as Pair)
  ...
  pairHourData.hourlyVolumeToken1 = pairHourData.hourlyVolumeToken0.plus(amount1Total)
  pairHourData.hourlyVolumeToken0 = pairHourData.hourlyVolumeToken1.plus(amount0Total)
